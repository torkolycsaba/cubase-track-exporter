"""
pre_cleanup.py — De-solo sweep + queue builder
-----------------------------------------------
Phase 1: scroll top to bottom, click any soloed buttons. Does NOT record states.
Phase 2: scroll back to top, take a clean scan, classify every track.

Two phases keeps state detection simple and reliable — no scroll-offset maths,
no duplicate detection across scroll positions.
...
def run_preflight(config, skip_desolo=False):
    cpr_path     = config["project_file"]
    pixel_per_scroll = config.get("pixel_per_scroll", 30)
    ...
    if skip_desolo:
        ...
        while True:
            ...
            for btn in soloed:
                print(f"  Soloed at ({btn['screen_x']}, {btn['screen_y']}) — clicking...")
                ...
                still        = find_soloed_buttons(fresh_region, search_x1, search_y1)
                if any(abs(b["screen_y"] - btn["screen_y"]) < 10 for b in still):
        ...
        print("Phase 1 complete — no tracks soloed.")
    ...
    while len(all_tracks) < TOTAL_TRACKS:
        ...
        if not all_tracks:
            # First frame — take everything visible
            all_tracks = list(visible)
            for i, b in enumerate(all_tracks):
                print(f"  Track {i+1:2d}: ({b['screen_x']}, {b['screen_y']})  "
                      f"state={b['state']}")
        else:
            # Only add buttons appearing below the last known track's
            # estimated current position (shifted up by scroll amount)
    ...
    # ── Pair names and build export queue ──────────────────────────────────────
...
if __name__ == "__main__":
    with open("config.json", "r") as f:
        config = json.load(f)
    skip = "--cleaned" in sys.argv
    try:
        export_queue = run_preflight(config, skip_desolo=skip)
