...
BEFORE RUNNING:
  ...
  - Run pre_cleanup first to confirm your export_queue is correct
  - Run this script

Run:
    python export_process.py
"""
...
import time
import json
from controller import check_control, start_listener, stop_listener
...
def find_next_selected_button(search_x1, search_y1, search_x2, search_y2,
                               done_y_positions, skip_radius=15):
    """
    Find the topmost selected S button that we have NOT already exported.

    done_y_positions:  list of CURRENT screen_y values for already-exported
                       buttons. These are updated after every scroll step
                       (subtract pixel_per_scroll each time), so they always
                       reflect where those buttons are right now on screen.
                       No offset math needed — just compare directly.
    ...
    for x, y in unique:
        ...
        already_done = any(
            abs(screen_y - done_y) < skip_radius
...
def run_export(export_queue, config):
    """Run the full export loop for all tracks in export_queue."""
    ...
    done_y_positions = []   # current screen_y of each exported button, kept up to date after every scroll
    ...
    for track_num, name in enumerate(export_queue, 1):
        ...
        while solo_pos is None:
            ...
            if solo_pos is None:
                pyautogui.scroll(-SCROLL_AMOUNT)
                time.sleep(SCROLL_SETTLE)
                # Shift every stored done_y up by one scroll step so they
                # stay accurate to where those buttons are now on screen.
                done_y_positions[:] = [y - pixel_per_scroll for y in done_y_positions]

        solo_btn_x, solo_btn_y = solo_pos
        print(f"  Found at ({solo_btn_x}, {solo_btn_y})")
        ...
        press_export_hotkey(export_hotkey)
        ...
        # Store current screen_y — updated after every scroll so always current
        done_y_positions.append(solo_btn_y)
        print(f"  Marked screen-Y={solo_btn_y} as done. Done list: {done_y_positions}")
...
if __name__ == "__main__":
    ...
    with open("config.json", "r") as f:
        config = json.load(f)
    # Standalone test queue — replace with real names from pre_cleanup
    test_queue = [
        "Guitar Left",
        "Guitar Right",
        "guitarrandom_right",
