...
# Format A: older projects
#   00 e7 00 00 00 00 00 00 | XX XX | 00 00 00 LL | <name> | 00 ef bb bf
_NAME_PATTERN = re.compile(
    b"\x00\xe7\x00\x00\x00\x00\x00\x00"   # 8-byte signature
    b".."                                    # 2-byte internal track ID
    b"\x00\x00\x00([\x01-\x50])"            # 4-byte length prefix (max 80 chars)
    b"([^\x00]{1,80})"                       # the track name string
    b"\x00\xef\xbb\xbf",                    # UTF-8 BOM terminator
    re.DOTALL,
)

# Format B: newer projects (seen in Cubase 14, MIDI/instrument tracks)
#   80 00 00 00 00 00 00 e7 | 00 00 00 00 | XX XX XX XX | 00 00 00 LL | <name> | 00 ef bb bf
#   The e7 marker is at byte 7 instead of byte 1, and there are 8 bytes
#   between signature and length field instead of 2.
_NAME_PATTERN_B = re.compile(
    b"\x80\x00\x00\x00\x00\x00\x00\xe7"  # 8-byte alternative signature
    b"........"                                    # 8 bytes (4 zero + 4 ID)
    b"\x00\x00\x00([\x01-\x7f])"             # 4-byte length prefix (max 127 chars)
    b"([^\x00]{1,80})"                            # the track name string
    b"\x00\xef\xbb\xbf",                       # UTF-8 BOM terminator
    re.DOTALL,
)
...
def get_audio_track_names(cpr_path: str | Path) -> list[str]:
    ...
    # Collect all matches from both format patterns, sorted by file offset
    # so we preserve the top-to-bottom track order regardless of which
    # format each track uses.
    all_matches = []
    for m in _NAME_PATTERN.finditer(data):
        all_matches.append((m.start(), m.group(2)))
    for m in _NAME_PATTERN_B.finditer(data):
        all_matches.append((m.start(), m.group(2)))
    all_matches.sort(key=lambda x: x[0])  # sort by file offset = track order

    for _, raw_bytes in all_matches:
